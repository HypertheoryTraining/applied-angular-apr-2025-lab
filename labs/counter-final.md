...
1. In the `src/counter-lab2` folder create a new directory called `pages` and in that folder, create a new component called `ui.ts`.
...
In the `src/counter-lab2/pages` folder add a new component called `prefs.ts`.

Create a child route in the `src/counter-lab2/counter.routes.ts` for this component, and add a link to it on the `counter.ts`.
